 ;(function(window, QB, app, CONFIG, $, Backbone) {
    ...
    $(function() {
        ...
        // $(document).on('click','.hellocallme', function(e) {
            //     alert("ssss");
            // })
            // $(document).on('submit','.j-join', function(e) {
            // alert("-ssddd");
            // e.preventDefaults();
            // var $form = $(this),
            ...
            app.helpers.join(JSON.parse(data)).then(function(user) {
                app.caller = user;
                var localData = JSON.parse(JSON.parse(localStorage.getItem('data')));

                updateUserCallId(localData.callURL,JSON.parse(data).login,user.id)
                ...
                QB.chat.connect({
                    ...
                    if(err) {
                        ...
                        console.log("---app.currentSession", app.currentSession)
        ...
        // $(document).on('click', '.j-user', function() {
            var $user = $(this),
                user = {
                    id: +$.trim( $user.data('callid') ),
                    name: $.trim( $user.data('username') )
                };
        ...
        // });
        /** Call / End of call */
        $(document).on('click', '.j-actions, .video-call, .call-chat', function() {
                    ...
                    if (err || !stream.getAudioTracks().length || (isAudio ? false : !stream.getVideoTracks().length)) {
                        ...
                        app.currentSession.call({}, function() {
                            if (!window.navigator.onLine) {
                                ...
                                if(isAudio) {
                                    $('.m-audio_call').addClass('hangup');
                                } else {
                                    $('.m-video_call').addClass('hangup');
                                }
        ...
        $(document).on('click', '.j-accept', function() {
            ...
            app.currentSession.getUserMedia(mediaParams, function(err, stream) {
                if (err || !stream.getAudioTracks().length || (isAudio ? false : !stream.getVideoTracks().length)) {
                    ...
                    console.log("---app.currentSession", app.currentSession)
                    console.log("-app.currentSession.initiatorID",app.currentSession.initiatorID)
                    console.log("-app.currentSession.opponentsIDs",app.currentSession.opponentsIDs)
                    ...
                    // if(userRole == 'Model') {
                    //     callAPIForCredit(app.currentSession.initiatorID, userID, callType);
                    // }
        ...
        // video call pause and mute options
