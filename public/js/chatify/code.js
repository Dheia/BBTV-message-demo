...
function attachmentTemplate(fileType, fileName, imgURL = null) {
  ...
  if (fileType != "image") {
    $data =
      '<div class="attachment-preview"><span class="fas fa-times cancel"></span><p style="padding:0px 30px;"><span class="fas fa-file"></span>' +
      escapeHtml(fileName) +
      "</p>";
    if (userRole != "Fan") {
      $data +=
        '<div class="number  mt-3 d-flex"><span class="minus1 " > - </span ><input name="price" class="input-price" type="text"id="premium_cost" value="0.00" /><span class="plus1 ml-1">+</span></div>';
    }
    $data += "</div >";
    return $data;
  } else {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        $(".messenger-infoView")
          ...
          .css(
            "background-image",
            'url("/profile-image/' + data.user_avatar + '")'
          );
        $(".header-avatar").css(
          "background-image",
          'url("/profile-image/' + data.user_avatar + '")'
        );
        $(".phone_number").attr("value", data.number);
        ...
        messageInput.focus();
        // update info in view
        $(".messenger-infoView .info-name").html(data.fetch.first_name);
        $(".m-header-messaging .user-name").html(data.fetch.first_name);
        $(".m-header-messaging .credit-info").html(data.wallet);
        // Star status
        data.favorite > 0
...
function getActiveList() {
  $(".messenger-active").html(avatarLoading(4));
  $.ajax({
    url: url + "/active",
    method: "POST",
    data: { _token: access_token },
    ...
    success: (data) => {
      if (data.count > 0) {
        $(".active-section").show();
        $(".messenger-active").html(data.active);
      } else {
        $(".active-section").hide();
      }
      // update data-action required with [responsive design]
...
function getSharedPhotos(user_id) {
  $.ajax({
    ...
    success: (data) => {
      ...
      $(".msg-sent-info").html(data.textMsg);
      $(".video-sent-info").html(data.videoMsg);
      $(".image-sent-info").html(data.imageMsg);
      $(".audio-sent-info").html(data.audioMsg);
      $(".audio-call-info").html(data.audioCall);
      $(".video-call-info").html(data.videoCall);
    },
    error: () => {
      console.error("Server error, check your response");
    },
...
$(document).ready(function () {
  ...
  $("body").on("click", ".messenger-list-item", function () {
    ...
    const id = $(".m-list-active").attr("data-contact");
    $(".Tip_model_id").attr("data", id);
  ...
  // video modal
  $("body").on("click", ".mediaopen", function () {
    $(this).find(".imageModalBox1").show();
  });
  ...
  // audio modal
  $("body").on("click", ".audioopen", function () {
    $(this).find(".imageModalBox2").show();
  });
  ...
  // $("body").on("click", ".video_pop", function () {
  // let src = $(this).css("background-image").split(/"/)[1];
  // $("#imageModalBox").show();
  // $("#imageModalBoxSrc").attr("src", src);
  // });
