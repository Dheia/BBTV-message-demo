...
function attachmentTemplate(fileType, fileName, imgURL = null) {
    ...
    if (fileType != "image") {
        $data = '<div class="attachment-preview"><span class="fas fa-times cancel"></span><p style="padding:0px 30px;"><span class="fas fa-file"></span>' + escapeHtml(fileName) + '</p>';
        if (userRole != 'Fan') {
            $data += '<div class="number  mt-3 d-flex"><span class="minus1 " > - </span ><input name="price" class="input-price" type="text"id="premium_cost" value="0.00" /><span class="plus1 ml-1">+</span></div>';
        }
        $data += '</div >';
        return ($data);
    } else {
    ...
    if (messenger != 0) {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                $(".messenger-infoView")
                    .find(".avatar")
                    .css("background-image", 'url("/profile-image/' + data.user_avatar + '")');
                $(".header-avatar").css(
                    "background-image",
...
$(document).ready(function () {
    ...
    $("body").on("click", ".messenger-list-item", function () {
        ...
        routerPush(document.title, `${url}/${userID}#login`);
