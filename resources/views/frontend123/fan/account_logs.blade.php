...
@if($item->method=='Tip')
Send Tip Amaount To {{$item->model->first_name ?? ''}} @endif </td>
...
<div class="mobile_card card text-white">
    <h6 class="mt-2 mb-2">@if($item->method=='Post unlock')
        Opened picture from {{$item->model->first_name ?? ''}} @endif
        @if($item->method=='Tip')
        Send Tip Amaount To {{$item->model->first_name ?? ''}} @endif</h6>
    <div class="row pb-2 account_log_border">
        ...
        <div class="col-lg-4 col-md-4 col-sm-4 col-4 ">50.00$</div>
    </div>
</div>
@endforeach
</div>
